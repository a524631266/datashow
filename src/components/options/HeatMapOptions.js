...
export const drawHeatmapOptions = (listdata, title, redrawEntityFunc, openInfo) => {
    // var title = "用电"
    ...
    return {
                ...
                load() {
                    ...
                    Highcharts.addEvent(xAxis.labelGroup.element, 'mouseover', (e) => {
                        ...
                        if ($(e.target)[0].tagName.toLowerCase() === "tspan") {
                            ...
                            key = $(e.target).parent().index(); // xAxisa 轴的位置
                            // console.log("tspan",index)
                        }
                        else {
                            brothers = $(e.target).parent().children();
                            // tslint:disable-next-line:radix
                            key = $(e.target).index(); // xAxisa 轴的位置
                        }
                        const len = brothers.length;
                        // 计算index位置为第几大
                        let index = 0;
                        for (let i = 0; i < len; i++) {
                            const domid = brothers[i];
                            let beforid = $(domid).attr("y");
                            let nowid = $(brothers[key]).attr("y");
                        ...
                        // 先隐藏 ###########
                        openInfo(categories[index], namemap[xlist[index]], clientX, clientY, e.target.getBoundingClientRect());
        ...
        yAxis: {
            ...
            labels: {
                ...
                formatter(a) {
                    ...
                    // tslint:disable-next-line:radix
                    const hourvalue = parseInt(this.value);
        ...
        tooltip: {
            ...
            pointFormatter() {
                ...
                const x = timemapx[this.options.x];
