...
import { RegionBoxChart, BoxChartTrans, RegionBoxChart2 ,ScatterChart} from '@/types/postreturnform';
...
const  testresolvedata = (datas: ScatterChart) => {
  // 分解数据头
  // tslint:disable-next-line:one-variable-per-declaration
  const entitylist = datas.linechart.entitylist;
  const valuelist = datas.linechart.valuelist;
  const map: any= {};
  valuelist.forEach(
    (arr: any) => {
      // const [ts, value, in] = arr;
      const ts = arr[0];
      const value= arr[1];
      const index = arr[2];
      const nextmap = map[ts];
      if(nextmap === undefined) {
        map[ts] = {};
        map[ts].datalist = [value];
        map[ts].nameindex = [index];
      } else {
        nextmap.datalist.push(value);
        nextmap.nameindex.push(index);
      }
    }
  );
  const mapts = Object.keys(map);
  const boxList = mapts.map(
    (ts) => {
      return map[ts].datalist;
    }
  );
  const ydata = (echarts as any).dataTool.prepareBoxplotData(boxList);
  // 2.同时计算ydata outliers 对应的配电柜信息
  // "outliers":[[0,-1.34],[2,-1.12]],
  const outliers = ydata.outliers;
  const outliersindex = outliers.map(
    (arr: any) => {
      // tslint:disable-next-line:one-variable-per-declaration
      const timeindex = arr[0],value = arr[1];
      const ind = map[mapts[timeindex]].datalist.indexOf(value);
      return map[mapts[timeindex]].nameindex[ind];
    }
  );
  // ydata.outliers = ydata.outliers.map(
  //   function(arr, index){
  //     return {
  //       x: arr[0], // 时间
  //       y: arr[1], // 值
  ...
  //   }
  // )
  ydata.axisData = mapts;
  ydata.outliersindex = outliersindex;
  ydata.entitylist = entitylist;
  return ydata;
};
...
export default class RegionBoxHighChartTestOnline extends Vue implements AxiosSourceManage {
    ...
    public getData() {
      ...
      const promise = getDataPromise<ScatterChart ,BoxChartTrans>(this.urlparas,PostPath.regionLineChart,this.axiosSource,this.dealData);
      promise.then(
    ...
    private dealData(data3: ScatterChart): BoxChartTrans {
      const data = testresolvedata(data3);
